"""Market Radar v1.16-E — Real Free API Multi-Asset Market Sync TG Test Send
with Safe Secret Preflight (One-Shot)

Extends v116D by adding a mandatory safe secret preflight step before any TG send.
The preflight only checks boolean presence of TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID
— it NEVER prints, reads, writes, or logs the actual values.
...
Key improvements over v116D:
  - Explicit safe_secret_preflight() step before any send attempt
  - Boolean-only credential reporting (true/false, no values)
  - redacted message proof (sha256 hash of message_id, never raw)
  - Does NOT record complete chat_id or token anywhere
...
Outputs:
  results/market_radar_v116e_real_free_api_multi_asset_raw_snapshots.json
  results/market_radar_v116e_real_free_api_multi_asset_signal_records.jsonl
  results/market_radar_v116e_real_free_api_multi_asset_card_records.jsonl
  results/market_radar_v116e_real_free_api_multi_asset_quality_gate_records.jsonl
  results/market_radar_v116e_real_free_api_multi_asset_send_readiness_records.jsonl
  results/market_radar_v116e_real_free_api_multi_asset_tg_send_attempts.jsonl
  results/market_radar_v116e_real_free_api_multi_asset_tg_test_send_result.json
  runs/market_radar/v116e_real_free_api_multi_asset_tg_test_card_preview.md
  runs/market_radar/v116e_real_free_api_multi_asset_tg_test_send_report.md
  runs/market_radar/v116e_real_free_api_multi_asset_tg_test_send_local_only_handoff.md

Usage:
    python scripts/run_market_radar_v116e_real_free_api_multi_asset_tg_test_send_with_safe_secret_preflight_one_shot.py
"""
...
VERSION = "v1.16-E"
STAGE = "v116e_real_free_api_multi_asset_tg_test_send_with_safe_secret_preflight_one_shot"
TASK_ID = "20260605_v116e_real_free_api_multi_asset_tg_test_send_rerun_with_safe_secret_preflight_one_shot"
...
RAW_SNAPSHOTS_JSON = OUTPUT_DIR / "market_radar_v116e_real_free_api_multi_asset_raw_snapshots.json"
SIGNAL_RECORDS_JSONL = OUTPUT_DIR / "market_radar_v116e_real_free_api_multi_asset_signal_records.jsonl"
CARD_RECORDS_JSONL = OUTPUT_DIR / "market_radar_v116e_real_free_api_multi_asset_card_records.jsonl"
QUALITY_GATE_JSONL = OUTPUT_DIR / "market_radar_v116e_real_free_api_multi_asset_quality_gate_records.jsonl"
SEND_READINESS_JSONL = OUTPUT_DIR / "market_radar_v116e_real_free_api_multi_asset_send_readiness_records.jsonl"
TG_SEND_ATTEMPTS_JSONL = OUTPUT_DIR / "market_radar_v116e_real_free_api_multi_asset_tg_send_attempts.jsonl"
SEND_RESULT_JSON = OUTPUT_DIR / "market_radar_v116e_real_free_api_multi_asset_tg_test_send_result.json"
CARD_PREVIEW_MD = RUNS_DIR / "v116e_real_free_api_multi_asset_tg_test_card_preview.md"
SEND_REPORT_MD = RUNS_DIR / "v116e_real_free_api_multi_asset_tg_test_send_report.md"
HANDOFF_MD = RUNS_DIR / "v116e_real_free_api_multi_asset_tg_test_send_local_only_handoff.md"
...
def safe_secret_preflight() -> dict:
    ...
    # SAFETY: NEVER print the actual values
    ...
    # If env vars are present, mark that we read them (established project pattern)
    if preflight_passed:
...
def build_multi_asset_snapshot(
    ...
    for sym in TARGET_ASSETS:
        ...
        volume_change_pct = float(spot.get("priceChangePercent", 0)) * 0.5
...
# ══════════════════════════════════════════════════════════════════════════
# Step 4: Render card (NO AI/model called)
# ══════════════════════════════════════════════════════════════════════════
...
def render_multi_asset_card(signal: dict) -> str:
    ...
    card_lines = [
        ...
        "",
        f"\U0001f510 v116E 安全预检通过 | 真实API | 测试群 one-shot 发送",
    ]
...
def run_send_readiness_gate(signal: dict, quality_gate: dict, preflight: dict) -> dict:
    ...
    print("\n[6] Running send-readiness gate (with preflight data)...")
...
def attempt_tg_test_send(
    ...
    """
    print("\n[7] Attempting TG test group send (one-shot, v116E safe send)...")
    ...
    try:
        ...
        if result.error_message:
            # Sanitize any possible credential leak in error message
        ...
        # Sanitize BEFORE printing — exception message may contain credentials
        error_str = str(e)
...
def write_outputs(
    ...
    elif not preflight_passed or not send_readiness.get("tg_sender_available"):
        audit_result = "real_free_api_card_ready_tg_blocked_missing_sender"
    else:
        audit_result = "blocked_gate_not_passed"
    ...
    result = {
        ...
        "tg_message_id_present": tg_attempt.get("message_id_present", False),
    ...
    # 11. Handoff markdown
    handoff_lines = [
        f"# Market Radar {VERSION} — Handoff: Real Free API Multi-Asset TG Test Send with Safe Secret Preflight",
        "",
        f"**Generated**: {generate_timestamp()}",
        f"**Task ID**: {TASK_ID}",
        f"**Run ID**: {RUN_ID}",
...
def main() -> int:
    ...
    print("with Safe Secret Preflight (One-Shot)")
    ...
    try:
        # ── Step 0: Safe Secret Preflight (v116E MANDATORY) ──
        ...
        # ── Step 4: Render card (NO AI) ──
        card_text = render_multi_asset_card(signal)

        # ── Step 5: Quality gate ──
        quality_gate = run_quality_gate(signal, card_text)

        # ── Step 6: Send-readiness gate (with preflight data) ──
        send_readiness = run_send_readiness_gate(signal, quality_gate, preflight)

        # ── Step 7: TG test send (with preflight validation) ──
        tg_attempt = attempt_tg_test_send(signal, card_text, send_readiness, preflight)

        # ── Step 8: Write outputs ──
        final_result = write_outputs(
            preflight, snapshot, signal, card_text,
            quality_gate, send_readiness, tg_attempt,
        )
    ...
    return 0  # Always exit 0 unless crash
