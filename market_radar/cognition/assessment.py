...
def build_assessment(event_id, event_title, lifecycle_state, expectation_gap, market_verdict, confidence_components, transmission_path_ids, evidence_ids, has_conflict=False):
    oc = sum(confidence_components.values()) / max(len(confidence_components), 1)
    conflicting = []
    if has_conflict:
        conflicting = ["unresolved_source_conflict"]
    return Assessment(assessment_id=sha256_id(["assessment", event_id]), event_id=event_id, event_summary=event_title[:200], lifecycle_state=lifecycle_state, expectation_gap=expectation_gap, market_confirmation=market_verdict, transmission_paths=transmission_path_ids, confidence_components=confidence_components, overall_confidence=round(oc, 4), supporting_evidence_ids=list(evidence_ids), contradicting_evidence_ids=conflicting, not_trading_instruction=True)
