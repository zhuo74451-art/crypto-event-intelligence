    ...
    for change in changes:
        ...
        if "flip" in ct:
            alert = WhaleAlertCandidate(
                ...
                dedup_key=WhaleAlertCandidate.compute_dedup_key("direction_flip", change.address, coin), severity=ALERT_RULES["direction_flip"]["severity"],
        ...
        if ct in REAL_OPEN_TYPES and change.current:
            ...
            if pos_value >= ALERT_RULES["large_new_position"]["min_value_usd"]:
                alert = WhaleAlertCandidate(
                    ...
                    dedup_key=WhaleAlertCandidate.compute_dedup_key("large_new_position", change.address, coin), severity=ALERT_RULES["large_new_position"]["severity"],
                    coin=coin, label=label,
                    address_short=addr_short,
        ...
        if "increase" in ct:
            ...
            if val_delta >= ALERT_RULES["large_increase"]["min_delta_usd"]:
                alert = WhaleAlertCandidate(
                    ...
                    dedup_key=WhaleAlertCandidate.compute_dedup_key("large_increase", change.address, coin), severity=ALERT_RULES["large_increase"]["severity"],
                    coin=coin, label=label,
                    address_short=addr_short,
        ...
        if "reduce" in ct:
            ...
            if val_delta >= ALERT_RULES["large_decrease"]["min_delta_usd"]:
                alert = WhaleAlertCandidate(
                    ...
                    dedup_key=WhaleAlertCandidate.compute_dedup_key("large_decrease", change.address, coin), severity=ALERT_RULES["large_decrease"]["severity"],
                    coin=coin, label=label,
                    address_short=addr_short,
                    observed_value=delta.get("position_value_delta_usd", 0),
        ...
        if change.current:
            ...
            if liq_dist is not None and liq_dist > 0 and liq_dist <= max_dist:
                alert = WhaleAlertCandidate(
                    ...
                    dedup_key=WhaleAlertCandidate.compute_dedup_key("liquidation_critical", change.address, coin), severity=ALERT_RULES["liquidation_critical"]["severity"],
                    coin=coin, label=label,
                    address_short=addr_short,
        ...
        if snap.leverage >= ALERT_RULES["high_leverage"]["min_leverage"]:
            ...
            if aid not in seen_ids:
                alerts.append(WhaleAlertCandidate(
                    ...
                    dedup_key=WhaleAlertCandidate.compute_dedup_key("high_leverage", snap.address, coin), severity=ALERT_RULES["high_leverage"]["severity"],
                    coin=coin, label=label,
                    address_short=addr_short,
        ...
        if snap.position_value_usd >= ALERT_RULES["concentrated_exposure"]["min_value_usd"]:
            ...
            if aid not in seen_ids:
                alerts.append(WhaleAlertCandidate(
                    ...
                    dedup_key=WhaleAlertCandidate.compute_dedup_key("concentrated_exposure", snap.address, coin), severity=ALERT_RULES["concentrated_exposure"]["severity"],
                    coin=coin, label=label,
                    address_short=addr_short,
