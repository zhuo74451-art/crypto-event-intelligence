...
NET_CONCENTRATION_RATIO = 0.8       # single direction >80% of gross
SINGLE_COIN_CONCENTRATION = 0.5     # single coin >50% of gross
SINGLE_ADDRESS_CONCENTRATION = 0.7  # single address >70% of gross
HIGH_WEIGHTED_LEVERAGE = 10.0
LIQ_CLUSTER_2PCT = 2.0
LIQ_CLUSTER_5PCT = 5.0
SAME_DIRECTION_THRESHOLD_USD = 1_000_000
RAPID_EXPANSION_PCT = 20.0
STALE_HOURS = 48
...
def rule_pr2_net_direction_concentration(
    ...
    """PR2: Net direction >80% of gross (extreme directional bias)."""
    if gross_exposure == 0:
        return None
    ...
    if long_ratio > NET_CONCENTRATION_RATIO or short_ratio > NET_CONCENTRATION_RATIO:
        ...
        return {
            ...
            "threshold": f"{NET_CONCENTRATION_RATIO * 100:.0f}% in one direction",
...
def rule_pr3_single_coin_concentration(
    ...
    """PR3: Single coin >50% of gross exposure."""
    findings = []
    if gross_exposure == 0:
        return findings
...
def rule_pr4_single_address_concentration(
    ...
    """PR4: Single address >70% of gross exposure."""
    findings = []
    if gross_exposure == 0:
        return findings
    ...
    for addr, value in addr_values.items():
        ratio = value / gross_exposure
        if ratio > SINGLE_ADDRESS_CONCENTRATION:
            findings.append({
                ...
                "threshold": f"> {SINGLE_ADDRESS_CONCENTRATION * 100:.0f}% in single address",
                "observed_value": round(ratio * 100, 1),
...
def rule_pr6_liquidation_cluster_2pct(
    ...
    """PR6: 2+ positions within 2% of liquidation."""
    count, total = compute_exposure_within_liq_pct(snapshots, LIQ_CLUSTER_2PCT)
    if count >= 2:
        affected = [s for s in snapshots
                    if s.liquidation_distance_pct is not None
                    and 0 < s.liquidation_distance_pct <= LIQ_CLUSTER_2PCT]
        return {
            "finding_id": _make_finding_id("PR6", snapshot_id, 0),
            "rule_id": "PR6_LIQUIDATION_CLUSTER_2PCT",
            "severity": SEVERITY_MAP["PR6"],
            "threshold": f"2+ positions within {LIQ_CLUSTER_2PCT}% of liquidation",
            "observed_value": count,
            "affected_addresses": [s.address for s in affected],
            "affected_coins": list({s.coin for s in affected}),
            "explanation": f"{count} positions (${total:,.0f}) within {LIQ_CLUSTER_2PCT}% of liquidation",
        }
    return None
...
def rule_pr7_liquidation_cluster_5pct(
    ...
    """PR7: 2+ positions within 5% of liquidation (includes those already in 2%)."""
    count, total = compute_exposure_within_liq_pct(snapshots, LIQ_CLUSTER_5PCT)
    if count >= 2:
        affected = [s for s in snapshots
                    if s.liquidation_distance_pct is not None
                    and 0 < s.liquidation_distance_pct <= LIQ_CLUSTER_5PCT]
        return {
            "finding_id": _make_finding_id("PR7", snapshot_id, 0),
            "rule_id": "PR7_LIQUIDATION_CLUSTER_5PCT",
            "severity": SEVERITY_MAP["PR7"],
            "threshold": f"2+ positions within {LIQ_CLUSTER_5PCT}% of liquidation",
            "observed_value": count,
            "affected_addresses": [s.address for s in affected],
            "affected_coins": list({s.coin for s in affected}),
            "explanation": f"{count} positions (${total:,.0f}) within {LIQ_CLUSTER_5PCT}% of liquidation",
        }
    return None
...
def rule_pr8_cross_whale_same_direction(
    ...
    """PR8: 2+ whales holding same coin in same direction with significant exposure."""
    findings = []
    ...
    for coin, dirs in coin_dir.items():
        for direction, positions in dirs.items():
            if len(positions) >= 2:
                ...
                if total >= SAME_DIRECTION_THRESHOLD_USD:
                    findings.append({
                        ...
                        "threshold": f"2+ whales same direction on same coin",
...
def rule_pr11_data_stale(
    ...
    """PR11: Positions with stale data (older than STALE_HOURS from reference)."""
    findings = []
    from market_radar.whale_domain.models import _iso_to_ts
    ref_ts = _iso_to_ts(reference_time)
    if ref_ts == 0:
        return findings
    stale_cutoff = ref_ts - (STALE_HOURS * 3600)
    for s in snapshots:
...
def evaluate_all_rules(
    ...
    # PR1
    f = rule_pr1_high_gross_exposure(gross, snapshot_id)
    if f:
        findings.append(f)

    # PR2
    f = rule_pr2_net_direction_concentration(long_exp, short_exp, gross, snapshot_id)
    if f:
        findings.append(f)

    # PR3
    findings.extend(rule_pr3_single_coin_concentration(snapshots, gross, snapshot_id))

    # PR4
    findings.extend(rule_pr4_single_address_concentration(snapshots, gross, snapshot_id))

    # PR5
    wl = compute_weighted_leverage(snapshots)
    f = rule_pr5_high_weighted_leverage(wl, snapshot_id)
    if f:
        findings.append(f)

    # PR6
    f = rule_pr6_liquidation_cluster_2pct(snapshots, snapshot_id)
    if f:
        findings.append(f)

    # PR7
    f = rule_pr7_liquidation_cluster_5pct(snapshots, snapshot_id)
    if f:
        findings.append(f)

    # PR8
    findings.extend(rule_pr8_cross_whale_same_direction(snapshots, snapshot_id))
    ...
    # PR10
    f = rule_pr10_rapid_exposure_expansion(gross, previous_gross, snapshot_id)
    if f:
        findings.append(f)

    # PR11
    if reference_time:
        findings.extend(rule_pr11_data_stale(snapshots, reference_time))
