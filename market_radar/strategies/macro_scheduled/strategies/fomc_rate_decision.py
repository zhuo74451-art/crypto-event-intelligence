"""FOMC Rate Decision Strategy — Interprets FOMC interest rate decisions."""
...
from market_radar.strategies.macro_scheduled.contracts.strategy_output import (
    MacroAssessmentProposal, AssessmentDirection,
)
...
class FomcRateDecisionStrategy:
    ...
    def run(self, event: EventInput) -> MacroAssessmentProposal | AbstentionDecision:
        ...
        transmission = self.transmission_builder.build_transmission_paths(
            ...
            composite.primary_direction.value if composite.primary_direction else "neutral"
        )
        confirmation = self.confirmation_engine.build_confirmation_snapshot({}, None)
        horizons = self.hypothesis_builder.build_horizon_assessments(composite, regime, transmission, confirmation)
        proposal = self.assessment_builder.build_proposal(
            proposal_id=f"fomc_{event.calendar.calendar_event_id}",
            release_event_id=event.calendar.calendar_event_id,
            as_of_time=datetime.utcnow(),
            surprise=macro_surprise, regime=regime,
            transmission=transmission, confirmation=confirmation,
            horizons=horizons,
        )
        return proposal
