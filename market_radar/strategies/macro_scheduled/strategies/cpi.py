# cpi.py strategy module
"""CPI Strategy - Full pipeline for Consumer Price Index releases."""
...
class CpiStrategy:
    """Full CPI strategy pipeline."""

    def __init__(self, strategy_version="macro_scheduled_strategy_v1.0.0"):
    ...
    def run(self, event):
        ...
        for act in event.actual_releases:
            exp_val = expectations.expected_value if expectations else None
            cs = self.surprise_engine.compute_component_surprise(
                act.component_id, act.actual_value, exp_val)
        ...
        macro_surprise = MacroSurprise(
            ...
            component_surprises=computed, composite=composite,
            computed_at=datetime.utcnow().isoformat())
        regime = self.regime_adapter.build_regime_context()
        direction = composite.primary_direction.value if composite.primary_direction else "neutral"
        transmission = self.transmission_builder.build_transmission_paths(
            EventFamily.CPI_HEADLINE, direction)
        ...
        proposal = self.assessment_builder.build_proposal(
            proposal_id="cpi_" + event.calendar.calendar_event_id,
            release_event_id=event.calendar.calendar_event_id,
            as_of_time=datetime.utcnow(), surprise=macro_surprise,
            regime=regime, transmission=transmission,
            confirmation=confirmation, horizons=horizons)
        return proposal
