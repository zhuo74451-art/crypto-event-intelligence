...
class FederalReserveAdapter(BaseAcquisitionAdapter):
    """Adapter for Federal Reserve RSS feeds.

    Reads from Fed RSS feeds (e.g., https://www.federalreserve.gov/feeds/press_all.xml).
    Inherits patterns from RssAdapter but uses direct feedparser integration
    to map: guid -> source_event_id, pubDate -> published_at, title, link, description.
    """
    ...
    def fetch(self, max_items: int = 10) -> AcquisitionAdapterResult:
        try:
            ...
            raise ImportError(
                "The 'feedparser' library is required for Federal Reserve RSS feeds. "
        ...
        if not url:
            result.errors.append(AcquisitionError(
                ...
                message="No RSS feed URL configured for Federal Reserve adapter",
        ...
        try:
            ...
            result.errors.append(AcquisitionError(
                ...
                message=f"Failed to fetch/parse Fed RSS feed: {exc}",
                source_id=self.source_id,
                url=url,
            ))
            return result
        ...
        if parsed.bozo and not parsed.entries:
            result.errors.append(AcquisitionError(
                code=AcquisitionErrorCode.PARSE_ERROR,
                message=f"Fed RSS parse error (bozo): {parsed.bozo_exception}",
                source_id=self.source_id,
                url=url,
            ))
            return result
        ...
        for i, entry in enumerate(parsed.entries):
            ...
            if published_dt is None:
                published_dt = self._parse_datetime(entry.get("updated_parsed"))
