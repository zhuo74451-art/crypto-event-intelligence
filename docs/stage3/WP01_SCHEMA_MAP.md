...
| `EventRecord` | Pydantic model | id, event_family, title, event_time, version, lifecycle_state |
...
| `ThesisRevision` | Pydantic model | id, thesis_id, version, previous_version, revision_body, revision_outcome, lifecycle_state |
...
| `HistoricalCaseManifest` | Pydantic model | id, case_id, event_family, market_regime, split_label, evidence_manifest_hash |
...
| `events` | `EventModel` | PK id |
| `event_revisions` | `EventRevisionModel` | PK id, FK event_id, UNIQUE(event_id, version) |
| `theses` | `ThesisModel` | PK id |
| `thesis_revisions` | `ThesisRevisionModel` | PK id, FK thesis_id, UNIQUE(thesis_id, version) |
...
| `historical_cases` | `HistoricalCaseModel` | PK id, UNIQUE case_id |
...
| File | Purpose |
|------|---------|
| `alembic/versions/cognition_v2_baseline.py` | Baseline schema — creates all 18 tables |
